...
def generate_full_hf(teapot_config, train_data, output):
    ...
    # Resolve the train_full_hf.py script path
    train_script = str(find_root() / "teapot" / "train_full_hf.py")
    ...
    lines = [
        ...
        "",
        f"export PYTHONPATH={find_root()}:${{PYTHONPATH:-}}",
        f"deepspeed --num_gpus {n_gpus} {train_script} \\",
        f"  --model {model_name} \\",
        f"  --data {train_data} \\",
        f"  --output {output_dir} \\",
